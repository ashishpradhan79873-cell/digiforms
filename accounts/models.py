...
class UserProfile(models.Model):
    ...
    graduation_subjects = models.TextField(blank=True)
...
class Vacancy(models.Model):
    ...
    hidden_from_users = models.BooleanField(default=False)
...
class PortalNews(models.Model):
    ...
    TARGET_CHOICES = [
        ...
        (TARGET_GOVERNMENT, "Government Portal"),
        (TARGET_STUDENT, "Student Portal"),
    ]
    ...
    news_type = models.CharField(max_length=20, choices=TYPE_CHOICES, default=TYPE_NOTICE)
    target_portal = models.CharField(max_length=20, choices=TARGET_CHOICES, default=TARGET_ALL)
...
class WalletTransaction(models.Model):
    ...
    tx_type = models.CharField(max_length=10, choices=TYPE_CHOICES, default=TYPE_ADD)
    ...
    def __str__(self):
        sign = "+" if self.tx_type == self.TYPE_ADD else "-"
        return f"{self.profile_id} {sign}{self.amount}"

    @staticmethod
    def balance_for_profile(profile):
        agg = profile.wallet_transactions.aggregate(
            add_total=Sum("amount", filter=models.Q(tx_type=WalletTransaction.TYPE_ADD)),
            spend_total=Sum("amount", filter=models.Q(tx_type=WalletTransaction.TYPE_SPEND)),
        )
